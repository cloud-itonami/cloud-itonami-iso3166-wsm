  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Apia Fabrication Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Apia Fabrication Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest procurement-authority-insufficient-is-held-and-unoverridable
  (testing "declared :claims-procurement-cleared? true but the claimed authority tier doesn't cover the declared contract value -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:procurement-authority-insufficient} (-> (store/ledger db) last :basis)))
...
(deftest tin-record-unverified-is-held-and-unoverridable
  (testing "unverified Ministry of Customs and Revenue TIN record when required -> HARD hold"
    (let [[db actor] (fresh)
