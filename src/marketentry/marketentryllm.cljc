(ns marketentry.marketentryllm
  ...
  the Independent State of Samoa (WSM) public-sector market-entry
  compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "claims-procurement-cleared?=" (:claims-procurement-cleared? e)
                        " procurement-category=" (:procurement-category e)
                        " declared-contract-value=" (:declared-contract-value e)
                        " claimed-approval-authority=" (:claimed-approval-authority e)
